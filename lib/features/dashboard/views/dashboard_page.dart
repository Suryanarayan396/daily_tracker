...
class DashboardPage extends StatelessWidget {
  ...
  void _showEditHitDialog(BuildContext context, LifeOSRepository repo) {
    final hitCtrl = TextEditingController(text: repo.highestImpactTask);
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
          ...
          if (state is DashboardInitial || state is DashboardLoadInProgress) {
            ...
            return RepositoryObserver(
              builder: (context, repo) {
                ...
                final salaryPercent = (repo.currentSalary / repo.targetSalary).clamp(0.0, 1.0);
                final totalWeeklyHours = repo.weeklyLearningHours.fold(0.0, (sum, val) => sum + val);
                final learningPercent = (totalWeeklyHours / 10.0).clamp(0.0, 1.0); // 10h target
                final careerPercent = (repo.applications.length / 5.0).clamp(0.0, 1.0); // 5 application target

                // Daily score configurations
                Color scoreColor = context.colorScheme.error;
                String scoreStatus = "Action Needed";
                if (repo.dailyScore >= 80) {
                  scoreColor = context.colorScheme.secondary;
                  scoreStatus = "Optimal Growth";
                } else if (repo.dailyScore >= 50) {
                    ...
                    child: Column(
                      ...
                      children: [
                                  ...
                                  children: [
                                    Row(
                                      ...
                                      children: [
                                        ...
                                        IconButton(
                                          onPressed: () => _showEditHitDialog(context, repo),
                                    ...
                                    Text(
                                      repo.highestImpactTask,
                                      maxLines: 3,
                                      overflow: TextOverflow.ellipsis,
                                      style: context.textTheme.bodyMedium?.copyWith(
                                        fontWeight: FontWeight.bold,
                                        color: Colors.white,
                                        decoration: repo.isHitCompleted ? TextDecoration.lineThrough : null,
                                      ),
                                    ),
                                    Row(
                                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                      children: [
                                        Text(
                                          repo.isHitCompleted ? 'Completed' : 'Pending',
                                          style: TextStyle(
                                            color: repo.isHitCompleted
                        ...
                        GridView.count(
                          ...
                          children: [
                            _buildKpiCard(context, 'Current Salary', currencyFormat.format(repo.currentSalary), Icons.account_balance_wallet_rounded, context.colorScheme.primary),
                            _buildKpiCard(context, 'Target Salary', currencyFormat.format(repo.targetSalary), Icons.track_changes_rounded, context.colorScheme.secondary),
                            _buildKpiCard(context, 'Liabilities / Debt', currencyFormat.format(repo.debt), Icons.payment_rounded, context.colorScheme.error),
                            _buildKpiCard(context, 'Emergency Fund', currencyFormat.format(repo.emergencyFund), Icons.health_and_safety_rounded, context.colorScheme.secondary),
                          ],
                        ),
                        SizedBox(height: context.screenHeight * 0.03),
                        ...
                        AppCard(
                          child: Column(
                            children: [
                              ...
                              SizedBox(height: context.screenHeight * 0.02),
                              _buildVectorProgress(context, 'Learning Deck (weekly)', learningPercent, context.colorScheme.secondary),
                              SizedBox(height: context.screenHeight * 0.02),
                              _buildVectorProgress(context, 'YouTube Content Velocity', 0.45, context.colorScheme.error),
                            ],
                          ),
                        ),
                        SizedBox(height: context.screenHeight * 0.03),
